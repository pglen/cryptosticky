...
/* =====[ FileDialogST.cpp ]========================================== 
   ...
   Description:     The CryptoSticky project, implementation of the FileDialogST.cpp                
...
UINT CALLBACK CFileDialogST::OFNHookProc(
...
{
	//P2N(_T("Called hook %s procedure win=%x\r\n"), num2msg(uiMsg), hdlg);
...
int CFileDialogST::DoModal()
{
	...
	//P2N(_T("Called CFileDialogST::DoModal()\r\n"));
...
{
	//AfxMessageBox(_T("INIT DIALOG"));

	//P2N(_T("Called init filedialog"));
...
CString CFileDialogST::GetFileDrive() const
{
	...
	// Split path into components
	//_tsplitpath_s(m_ofn.lpstrFile, szDrive, MAX_PATH, 
	//		NULL, MAX_PATH, NULL, MAX_PATH, NULL, MAX_PATH);
...
CString CFileDialogST::GetNextPathName(POSITION& pos) const
{
	...
	if (lpsz == m_ofn.lpstrFile) // first time
	{
		...
		// find TCHAR pos after first Delimiter
	...
	// find TCHAR pos at next Delimiter
	while(*lpsz != chDelimiter && *lpsz != '\0')
		lpsz = _tcsinc(lpsz);
