								...
								int iTransPixelX,  int iTransPixelY  )
{
	...
	HDC hdcSrc = CreateCompatibleDC(GetDC(HWND_DESKTOP));	ASSERT(hdcSrc);
	HDC hdcDst = CreateCompatibleDC(GetDC(HWND_DESKTOP));	ASSERT(hdcDst);
							...
							
{
	...
	HDC hdcDst = CreateCompatibleDC(GetDC(HWND_DESKTOP));	ASSERT(hdcDst);
	HBITMAP hBmpMask = CreateBitmap(ww, hh, 1, 1, NULL); ASSERT(hBmpMask);
