	...
	m_dwCurrentFilePointer(0)
{
	...
	// Not a BOM mark - it's an ANSI file
...
BOOL CUTF16File::Open(LPCTSTR lpszFileName, UINT nOpenFlags, CFileException* pError /*=NULL*/)
{
	...
	bResult = CStdioFile::Open(lpszFileName, nOpenFlags | CFile::typeBinary, pError);
...
LPTSTR CUTF16File::ReadUnicodeString(LPTSTR lpsz, UINT nMax)
{
	...
	while(!m_Accumulator.empty() && --nMax)
	{
		bRead = TRUE;        
...
VOID CUTF16File::WriteString(LPCTSTR lpsz, BOOL bAsUnicode /*= FALSE */)
{
	...
	else
	{
		...
		WriteANSIString(T2A((LPTSTR)lpsz));
#endif
	}
}
...
BOOL CUTF16File::LoadAccumulator()
{
	...
	// CStdioFile::Read() returns number of bytes read.....
	UINT uCount = CStdioFile::Read(cbBuffer, ACCUMULATOR_CHAR_COUNT * sizeof(WCHAR));

	WCHAR* pwszBuffer = reinterpret_cast<WCHAR*>(cbBuffer);

	// uCount is number of bytes, but we look at WCHARs, which are 2 bytes - hence, loop length is uCount / 2
...
ULONGLONG CUTF16File::Seek(ULONGLONG lOff, UINT nFrom)
{
	ULONGLONG lResult = CStdioFile::Seek(lOff, nFrom);
	m_dwCurrentFilePointer = CStdioFile::Seek(0UL, CFile::current);
...
VOID CUTF16File::WriteANSIString(LPCSTR lpsz)
{
	// Cast to (LPCSTR) by JW (C2664 Error)
	CString str(lpsz);
 	CFile::Write(lpsz, str.GetLength());
}
