...
/* =====[ XrayM1.cpp ]========================================== 
   ...
   Description:     The xraynotes project, implementation of the XrayM1.cpp                
...
// XrayM1.cpp : implementation file
//
	...
	: CGrayDlg(CXrayM::IDD, pParent)
{
	...
	//dlgbackcol	= GetSysColor(COLOR_MENU);
...

{
	...
	int hhh = (hh + 2 * gap) * (counti) + (2 * gap /2) * (counts) 
						+ 2 * topgap;
...

{
	...
	if(hRsrc == NULL)
		{	
		CString tmp; 
		tmp.Format("Could not open menu resource %d", lpszResourceName);
		AfxMessageBox(tmp);
		return FALSE;
		}
	...
	if(hGlobal == NULL)
		{
		CString tmp; 
		tmp.Format("Could not load menu resource %d", lpszResourceName);
		return FALSE;
		}	
